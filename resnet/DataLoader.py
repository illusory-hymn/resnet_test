...
def load_test_bbox(root, test_gt_path):
    test_gt = []
    test_txt = []
    resize_size = 224
    with open(test_gt_path, 'r') as f:
        ...
        for line in f:
            ...
            for i in range(box_num):
                bbox[4*i], bbox[4*i+1], bbox[4*i+2], bbox[4*i+3] = bbox[4*i]/w*resize_size, bbox[4*i+1]/h*resize_size , bbox[4*i+2]/w*resize_size , bbox[4*i+3]/h*resize_size
...
class ImageDataset(data.Dataset):
    def __init__(self, args):
        ...
        self.label_classes = [] ## 存储classes名字
        for k, v in self.img_dataset.class_to_idx.items():
            self.label_classes.append(k)
        self.img_dataset = self.img_dataset.imgs  ## 得到数据集每个图片的完整路径和类别编号
        self.test_bbox = load_test_bbox(self.root, self.test_gt_path) ## 得到测试集路径和bbox
